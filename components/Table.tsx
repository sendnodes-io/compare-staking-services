...
import {
    ...
    ArrowTopRightOnSquareIcon, CheckBadgeIcon,
    ...
    if (isLoading) {
        return (
            <div className="flex justify-center items-center h-64 mt-8">
                <div
                    className="animate-spin rounded-full h-32 w-32 border-b-2 border-gray-900 notdark:border-white"></div>
            </div>
        );
    }
            ...
            <div className="mt-8 flex flex-col items-center w-full">
                <div className="-my-2 overflow-x-auto  sm:max-w-full sm:px-8 w-full max-w-[90vw] ">
                    <div className="inline-block min-w-full py-2 align-middle px-1">
                        <div
                            className="overflow-x-hidden overflow-y-auto shadow ring-1 ring-black ring-opacity-5 rounded-lg max-h-[80vh]">
                                            ...
                                            <td className="whitespace-nowrap px-3 py-4 text-sm ">
                                                <div className="flex items-center justify-start gap-x-2">
                                                    <div className="h-full w-12 flex-shrink-0 items-center">
                                                        {!!params.logo_url && (
                                                            <div
                                                                className="h-12 w-12 bg-neutral-500 rounded-full flex justify-center items-center">
                                                                <img
                                                    ...
                                                    <div className="">
                                                        <div className="font-medium flex items-center">
                                                            {params.name}
                                                            <VerifiedBadge verified={!!params.feature_verified}/>
                                                        </div>
                                                                ...
                                                                .map(
                                                                    (feature) =>
                                                                        ...
                                                                        !!(params as any)[feature.key] && (
                                                                            <FeatureItem feature={feature} params={params}/>
                                                                        )
                                                                )}
                                                        </p>
                                                    </div>
...
function FeatureItem({feature, params}: { feature: Feature, params: NodeParams }) {
    ...
    <div className="flex-inline cursor-pointer">
        ...
        <div className="relative flex flex-col items-center group cursor-pointer ml-1">
            ...
            >
                ...
                {!verified && <QuestionMarkCircleIcon
                    className="h-5 w-5 inline-flex align-top text-gray-500 notdark:text-gray-500"/>}
            </button>
            <div className="absolute bottom-0 flex-col items-center hidden mb-4 group-hover:flex ">
                                            <span
        ...
        <Modal open={showModal} setOpen={setShowModal}>
            <div className={`mx-auto flex h-12 w-12 items-center justify-center rounded-full ${verified && `bg-green-100`}`}>
                {verified &&
                    <CheckBadgeIcon className="h-10 w-10 text-[#3A9C90] notdark:text-[#3A9C90]"/>}
                {!verified && <QuestionMarkCircleIcon
                    className="h-10 w-10 text-gray-500 notdark:text-gray-500"/>}
            </div>
            <div className="mt-3 text-center sm:mt-5">
                <Dialog.Title as="h3" className="text-lg font-medium leading-6 text-gray-900">
                    {verified ? 'Verified' : 'Unverified'}
                ...
                <div className="mt-2">
                    <p className="text-sm text-gray-500">
                        {verified ? 'This staking service provider has been verified.' : 'This staking service provider has not been verified.'}
                        {!verified && (
                            <p>Please reach out to us on Telegram to verify this staking service provider.</p>
                        )}
                    </p>
                </div>
            </div>
